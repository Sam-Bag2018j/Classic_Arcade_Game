...
var Enemy = function (x, y, speed) {
    ...
    // variables x,y are used to determine the x and y axis and speed of the enemy
    this.x = x;
    this.y = y;
    this.speed = speed;
...
// render the image of player
Player.prototype.render = function () {
    ctx.drawImage(Resources.get(this.player), this.x, this.y);
};

// user can use the arrow keys to move 
Player.prototype.handleInput = function (keyPress) {

    // Enables user on left arrow key to move left on the x axis by 102
    // allow user not to go off the game tiles on the left side
   ...
   if(lives > 0){
   //check if user reaches to water area
...
console.log("we reach to desired area");
    document.getElementById('score').innerHTML=  scores +"scores";
     ...
     console.log("lives still above zero");
   // }
     //when the user win the game 
      if(lives > 0 && scores > 5)
     {
       
     var t =confirm("Congatulation you win!!!, do you want to play again?");
...
// each enemy located on the y axis from 0 on the x axis move at a speed of 205 
enemyLocation.forEach(function (locationY) {
    enemy = new Enemy(0, locationY, 205);
    allEnemies.push(enemy);
});

// The postion of the player is located at x=200,and y=405
